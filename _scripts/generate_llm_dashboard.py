...
<div class="chart-card">
  ...
  <p class="chart-legend">The X axis shows cost per task (USD), with log scale on by default. Use the toggle to compare log and linear views. Hover a point for details; click to highlight the row in the table. Models without published cost-per-task data are listed in the table but not plotted. The green line is the <strong>best-in-class frontier</strong>: each diagonal segment connects successive models where no cheaper model scores higher.</p>
  <button class="btn" id="fScale" type="button" aria-pressed="true">Log X axis: On</button>
</div>
...
function renderChart(bic) {
  ...
  // best-in-class frontier as a green diagonal line through the top-left corner
  if (bic.list.length) {
    datasets.push({
      type: "line",
...
function initControls() {
  ...
  $("fScale").onclick = () => {
    logX = !logX;
    $("fScale").textContent = "Log X axis: " + (logX ? "On" : "Off");
    $("fScale").setAttribute("aria-pressed", String(logX));
    render();
  };
  $("fReset").onclick = () => {
