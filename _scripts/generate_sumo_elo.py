...
function renderChart(){
  ...
  selected.forEach((id,si)=>{
    ...
    const data = labels.map((_,k)=>byIdx.has(chartFrom+k) ? byIdx.get(chartFrom+k) : null);
  ...
  $("sep-note").textContent = selected.length
    ? "The x-axis spans the selected rikishi's available dates; gaps are bashos where a selected rikishi was not on a banzuke (kyujo / retirement)."
