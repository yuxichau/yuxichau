...
<div class="panel">
  ...
  <div class="controls">
    ...
    <span class="chips" id="sep-chips"></span>
  </div>
...
q.addEventListener("input", ()=>{
  ...
  hits.forEach(r=>{ const d=document.createElement("div");
    d.textContent = `${r[1]} — debut ${bashos[r[2]]} · peak ${r[3]}`;
...
function renderChart(){
  const labels = bashos.slice(fromIdx, toIdx+1);
  ...
  selected.forEach((id,si)=>{
    ...
    const byIdx = new Map();
    for (let i=0;i+2<s.length;i+=3){ const bi=s[i]; if (bi>=fromIdx && bi<=toIdx) byIdx.set(bi, s[i+1]); }
    const data = labels.map((_,k)=>byIdx.has(fromIdx+k) ? byIdx.get(fromIdx+k) : null);
  ...
  $("sep-note").textContent = selected.length
    ? "Gaps = bashos where the rikishi was not on a banzuke (kyujo / retirement)."
...
function ratingAt(id, idx){
  ...
  let best = null;
  for (let i=0;i+2<s.length;i+=3){ if (s[i] > idx) break; best = [s[i+1], s[i+2]]; }
  return best;
}
function renderLb(){
  const rows = [];
  for (const r of rikishi){
    const at = ratingAt(r[0], toIdx), st = ratingAt(r[0], fromIdx);
    if (!at) continue;
...
(function init(){
  ...
  ["Hakuho","Terunofuji","Aonishiki","Kotozakura"].forEach(n=>{
    for (const r of rikishi) if (r[1]===n){ addRikishi(r[0]); break; }
  });
  renderLb();
})();
})();
