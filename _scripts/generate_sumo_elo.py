...
function renderChips(){
  ...
  selected.forEach(id=>{ const r = ro(id), off=disabled.has(id);
    ...
    const toggle=document.createElement("button"); toggle.className="chip-toggle"; toggle.type="button"; toggle.textContent=r[1]; toggle.title=off ? "Show on chart" : "Hide from chart"; toggle.setAttribute("aria-pressed",String(!off)); toggle.onclick=()=>toggleRikishi(id);
    const remove=document.createElement("button"); remove.className="chip-remove"; remove.type="button"; remove.textContent="×"; remove.title="Remove filter"; remove.setAttribute("aria-label",`Remove ${r[1]}`); remove.onclick=()=>removeRikishi(id);
