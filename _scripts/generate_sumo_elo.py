...
#sep-app .chart-options { display:flex; flex-wrap:wrap; gap:.8rem; align-items:center; margin:0 0 .7rem; font-size:.82rem; color:#57606a; }
#sep-app .chart-options label { display:inline-flex; align-items:center; gap:5px; }
#sep-app .chart-options input[type=range] { width:120px; }
#sep-app .chart-options output { min-width:3.2em; color:#1f2328; font-variant-numeric:tabular-nums; }
...
#sep-app .chip.off { background:#f6f8fa; color:#8c959f; border-color:#d0d7de; }
#sep-app .chip .chip-toggle { border:none; background:none; color:inherit; cursor:pointer; font-size:.82rem; padding:0; }
#sep-app .chip .chip-remove { border:none; background:none; color:#57606a; cursor:pointer; font-size:1rem; line-height:1; padding:0 2px; }
...
<div class="panel">
  ...
  <div class="controls">
    <div class="searchbox">
      <input type="text" id="sep-q" placeholder="Search rikishi in English (e.g. Hakuho, Terunofuji, Aonishiki...)" autocomplete="off">
      <div class="search-results" id="sep-qr"></div>
    </div>
  </div>
  <div class="chart-options" aria-label="Chart display options">
    <label><input type="checkbox" id="sep-smooth"> LOWESS smoothing</label>
    <label for="sep-span">Parameter <input type="range" id="sep-span" min="0.05" max="0.80" step="0.01" value="0.20"><output id="sep-span-value">20%</output></label>
    <label><input type="checkbox" id="sep-jp"> Japanese rikishi names (where available)</label>
  </div>
...
let selected = [], disabled = new Set(), activeDims = [true,true,true,true,true,true];
let view = "raw", smoothEnabled = false, lowessSpan = 0.20, jpNames = false;
function displayName(r){ return (jpNames && r?.[4]) ? r[4] : (r?.[1] ?? ""); }
function lowess(data, span){
  const out=data.slice(); let start=0;
  while(start<data.length){
    while(start<data.length && data[start]==null) start++;
    if(start>=data.length) break;
    let end=start; while(end<data.length && data[end]!=null) end++;
    const n=end-start, k=Math.max(2,Math.ceil(span*n));
    for(let i=start;i<end;i++){
      const ns=[]; for(let j=start;j<end;j++) ns.push([Math.abs(j-i),j,data[j]]); ns.sort((a,b)=>a[0]-b[0]);
      const h=ns[Math.min(k-1,ns.length-1)][0]||1; let sw=0,sx=0,sy=0,sxx=0,sxy=0;
      ns.slice(0,k).forEach(([dist,j,y])=>{ const u=dist/h,w=Math.pow(1-u*u*u,3); sw+=w;sx+=w*j;sy+=w*y;sxx+=w*j*j;sxy+=w*j*y; });
      const den=sw*sxx-sx*sx; out[i]=den ? (sy*sxx-sx*sxy+(sw*sxy-sx*sy)*i)/den : sy/sw;
    }
    start=end;
  }
  return out;
...
const smoothCtl=$("sep-smooth"), spanCtl=$("sep-span"), jpCtl=$("sep-jp"), spanValue=$("sep-span-value");
smoothCtl.addEventListener("change", e=>{ smoothEnabled=e.target.checked; renderChart(); });
spanCtl.addEventListener("input", e=>{ lowessSpan=+e.target.value; spanValue.textContent=Math.round(lowessSpan*100)+"%"; if(smoothEnabled) renderChart(); });
jpCtl.addEventListener("change", e=>{ jpNames=e.target.checked; renderChips(); renderChart(); renderLb(); renderCalibratedLb(); });
...
function addRikishi(id){
  ...
  selected.push(id);
  disabled.delete(id);
  if (selected.length > 8) { disabled.delete(selected[0]); selected.shift(); }
  renderChips(); renderChart();
}
function removeRikishi(id){ selected = selected.filter(x=>x!==id); disabled.delete(id); renderChips(); renderChart(); }
function toggleRikishi(id){ if(disabled.has(id)) disabled.delete(id); else disabled.add(id); renderChips(); renderChart(); }
function renderChips(){
  const c = $("sep-chips"); c.innerHTML = "";
  selected.forEach(id=>{ const r = ro(id), off=disabled.has(id);
    const sp = document.createElement("span"); sp.className=`chip${off ? " off" : ""}`;
    const toggle=document.createElement("button"); toggle.className="chip-toggle"; toggle.type="button"; toggle.textContent=displayName(r); toggle.title=off ? "Show on chart" : "Hide from chart"; toggle.setAttribute("aria-pressed",String(!off)); toggle.onclick=()=>toggleRikishi(id);
    const remove=document.createElement("button"); remove.className="chip-remove"; remove.type="button"; remove.textContent="×"; remove.title="Remove filter"; remove.setAttribute("aria-label",`Remove ${displayName(r)}`); remove.onclick=()=>removeRikishi(id);
    sp.append(toggle,remove); c.appendChild(sp);
  });
}
...
function renderChart(){
  const plotted = selected.filter(id=>!disabled.has(id));
  ...
  mainChart.options.plugins.legend.display = plotted.length>1 && plotted.length<=8;
  mainChart.data = { labels, datasets: [] };
  plotted.forEach((id,si)=>{
    ...
    const plottedData = smoothEnabled ? lowess(data, lowessSpan) : data;
    mainChart.data.datasets.push({ label: displayName(ro(id)) || String(id), data:plottedData,
  ...
  mainChart.update();
  $("sep-note").textContent = plotted.length
    ...
    : (selected.length ? "All selected filters are hidden. Use the controls below the graph to show a line or remove a filter." : "Search and add rikishi above to plot their Elo curves.");
}
...
function renderLb(){
  ...
  for (const r of rikishi){
    ...
    if (!inWin) continue;
    rows.push({ id:r[0], name:displayName(r), div:at[1], elo:at[0], delta:st ? at[0]-st[0] : null, peak:r[3] });
  }
...
function renderCalibratedLb(){
  ...
  for(const r of rikishi){ const s=series[r[0]]; if(!s) continue; let best=null;
    ...
    if(best) rows.push({name:displayName(r),id:r[0],elo:best.elo,idx:best.idx,raw:best.raw});
  }
