...
<div class="foot"><p><strong>Coverage.</strong> This page includes current Go models for which the free Artificial Analysis v4.3 feed exposes both an Intelligence Index and a cost-per-task value. Qwen3.6 Plus remains unplotted because the current AA feed does not expose a matching cost-per-task value. Some newly listed Go models, including Qwen3.8 Flash, LongCat-2.0, MiMo-V2.6-Flash, and GPT 6 Luna, also lack a matching AA row in the current feed. Scores, prices, and the Go model list can change; regenerate from the vendored refresh script.</p></div>
<script>__CHART__
