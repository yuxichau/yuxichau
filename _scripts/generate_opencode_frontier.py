...
<div class="foot"><p><strong>Coverage.</strong> This page includes Go models for which the current free Artificial Analysis v4.3 feed exposes both an Intelligence Index and a cost-per-task value. GPT 5.6 Luna is now included when its max-effort AA row is available. Qwen3.6 Plus remains unplotted because the current AA feed does not expose a matching cost-per-task value. GLM-5.3 and GLM 5.3 Flash appear in Artificial Analysis, but are not in the current OpenCode Go model list, so they are not treated as Go models here. Scores and prices can change; regenerate from the vendored refresh script.</p></div>
<script>__CHART__
