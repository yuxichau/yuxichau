...
<div class="card"><div class="chart"><canvas id="chart"></canvas></div><p class="legend">The green line is the frontier: after sorting by Go usage share, each point is retained when no cheaper model has a higher Intelligence Index. The x axis is logarithmic because the estimated contributions span several orders of magnitude. Hover points for the model, proxy cost, and quota contribution.</p></div>
...
<div class="foot"><p><strong>Coverage.</strong> The current OpenCode Go documentation lists 33 named models, with peak/off-peak price rows for some models. This page plots the 17 models for which the current Artificial Analysis v4.3 free feed exposes both an Intelligence Index and a cost-per-task value using the exact or closest documented model match. The chart uses the OpenCode Go off-peak price where the documentation gives separate peak and off-peak rows; peak prices are shown in the source table but not combined into a second point. The remaining Go models are retained in the audit but omitted from the graph when AA has no comparable cost-per-task record. DeepSeek V4.1 Flash is now included as a distinct model, matched to AA's <em>DeepSeek V4.1 Flash (Reasoning, Max Effort)</em>, II 39.5. It is not the same model as DeepSeek V4 Flash, matched to AA's <em>DeepSeek V4 Flash 0731</em>, II 34.3. Scores, prices, and the Go model list can change; regenerate from the vendored refresh script.</p></div>
<script>__CHART__
