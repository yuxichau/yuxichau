...
This is the sequel to yesterday's [quant quality ladder](/posts/the-quant-quality-ladder/), where a 27B model fell off a cliff at 1.8 bits. The caveat there was that architecture reshapes where the cliff sits, and a 770B MoE with per-layer allocation demonstrates it: MoE redundancy buys back precision a dense model cannot spare.

Frontier-class models are now built in three steps.

The first step is the architecture, shaped by two pressures: memory and signal. Mixture of experts keeps knowledge large while active parameters stay small. The [MLA trick from DeepSeek-V2](https://arxiv.org/abs/2405.04434) shrinks the memory that context eats, and linear attention with sparse indexers avoids the quadratic bill of full attention. Routing, position encodings and objectives all make computation do more per token. Efficiency is decided here, before training starts.
...
The third step is compression: make the model cheap to run and easy to move. Quantization from 8-bit down to the 1.3-bit class, sometimes extremely precise, as with Tencent's per-layer allocation, plus distillation, pruning, everything that ends in a [GGUF](https://github.com/ggml-org/llama.cpp) someone can download.

The pattern I keep noticing: the interesting innovation happens in steps one and three, and it is led by the open-source ecosystem. DeepSeek's MoE and latent attention, Qwen and GLM shipping architecture in the open, llama.cpp's quant formats from [AWQ](https://github.com/mit-han-lab/llm-awq) to sparse ternary, Hy4 itself under Apache 2.0. Step two is where the money goes and where the least is published: data and training runs stay private.
...
It is the good kind of race. Each trick gets tested by thousands of people the moment it lands, which is how a 200GiB frontier model becomes a download instead of a machine-room decision. Less secrecy, more floor, and the floor is what the rest of us stand on.